...
% Control if contact force is estimated on a test trajectory or a 
% trajectory supplied by a file
...
showToolPoints = true;
% Display test trajectory
showTestTrajectory = true;
% Create tool on test trajectory video 
makeTestTrajectoryVideo = false;
fileName = 'finalTestTrajectory.avi';
...
orgCloud = pcread('pointClouds/objectPointCloudSimplified.ply');
...
% SAVE TRAJECTORY IN STRUCTURE USED BY THIS SECTION DENPENDING ON CHOICE

% Procedure:
% Step 1: Iterate through all trajectory points
...
if makeTestTrajectoryVideo == true
    close all
...
% Matrix to store the points within the radius constraint
pointsInRange = [];
